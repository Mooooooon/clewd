...
const {Main, AI, genericFixes, encodeDataJSON, indexOfAny, parseEvent, checkResErr, cleanJSON} = require('./clewd-utils'), {DangerChars} = require('./clewd-message'), Decoder = new TextDecoder('utf-8');
...
class ClewdStream extends TransformStream {
    ...
    async #handle(chunk, controller) {
        if ('string' != typeof chunk) {
            ...
            // 使用 stream 选项确保正确处理大型或多字节字符串
            chunk = Decoder.decode(chunk, {'stream': true});
        } else {
            this.#recvLength += Buffer.byteLength(chunk, 'utf8');
        }
        // 处理事件标记和回车符，避免编码问题
        this.#compRaw += chunk.replace(/event: [\w]+\s*|\r/gi, '');
    ...
    async #parseBuf(event, controller) {
        ...
        try {
            ...
            parsed = JSON.parse(cleanJSON(eventData));
            ...
            if (parsed.completion || parsed.delta?.text || parsed.choices?.[0]?.delta?.content) {
                parsed.completion = genericFixes(parsed.completion || parsed.delta?.text || parsed.choices?.[0]?.delta?.content);
